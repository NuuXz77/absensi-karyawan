...
class ExportPdf extends Component
{
    ...
    public function export()
    {
        ...
        $karyawanInfo = null;
        if ($this->karyawan_id) {
            $karyawanInfo = Karyawan::find($this->karyawan_id);
        }

        $filename = $this->nama_file . '.pdf';

        $this->dispatch('show-toast', type: 'success', message: 'PDF sedang digenerate...');
        
        // Generate PDF dengan DomPDF langsung
        $html = view('exports.absensi-pdf', [
            'absensis' => $absensis,
            'periodeText' => $this->periodeText,
            'karyawanInfo' => $karyawanInfo,
            'tanggal_export' => Carbon::now()->locale('id')->isoFormat('DD MMMM YYYY, HH:mm'),
        ])->render();
    ...
    public function render()
    {
        ...
        return view('livewire.admin.absensi.modals.export-pdf', [
            'karyawans' => $karyawans,
        ]);
    }
}
