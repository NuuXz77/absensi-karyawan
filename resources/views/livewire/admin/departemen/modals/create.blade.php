<div x-data="modal('modal_create_departemen')">
    <button class="btn btn-primary btn-sm gap-2" @click="openModal()">
        <x-heroicon-o-plus class="w-5 h-5" />
        <span class="hidden sm:inline">Tambah Departemen</span>
    </button>
    @teleport('body')
        <dialog id="modal_create_departemen" class="modal" wire:ignore.self x-show="open">
                ...
                <form wire:submit.prevent="save">
                    <div class="grid grid-cols-2 gap-4 sm:grid-cols-1 lg:grid-cols-2">
                        ...
                        <fieldset>
                            ...
                            <label
                                class="input w-full validator input-bordered flex items-center gap-2 @error('kode_departemen') input-error @enderror">
                                <x-heroicon-o-hashtag class="w-4 h-4 opacity-70" />
                                <input required type="text" wire:model="kode_departemen" placeholder="Contoh: HR"
                                    class="uppercase" maxlength="50" />
                            </label>
                        ...
                        <fieldset>
                            ...
                            <label
                                class="input w-full validator input-bordered flex items-center gap-2 @error('nama_departemen') input-error @enderror">
                                <x-heroicon-o-building-office class="w-4 h-4 opacity-70" />
                                <input required type="text" wire:model="nama_departemen"
    ...
    <x-partials.toast 
        :success="$showSuccess ? 'Departemen berhasil ditambahkan!' : null" 
