<div>
    <dialog id="modal_delete_departemen" class="modal" wire:ignore.self>
        <div class="modal-box border border-base-300">
            ...
            <div class="py-4">
                <p class="text-base mb-4">Apakah Anda yakin ingin menghapus departemen ini?</p>
                
                @if($departemen)
                    <div class="bg-base-200 p-4 rounded-lg space-y-2">
                        <div class="flex items-center gap-2">
                            <x-heroicon-o-building-office class="w-5 h-5 opacity-70" />
                            <span class="font-semibold">{{ $departemen->nama_departemen }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span>{{ $departemen->karyawans_count ?? 0 }} Karyawan</span>
                        </div>
                    </div>
                    
                    @if($departemen->karyawans_count > 0)
                        <div class="alert alert-warning mt-4">
                            <x-heroicon-o-exclamation-triangle class="w-5 h-5" />
                            <span>Departemen ini memiliki {{ $departemen->karyawans_count }} karyawan. Pastikan untuk memindahkan karyawan terlebih dahulu!</span>
                        </div>
                    @endif
                @endif
    ...
    <div class="toast toast-start z-[9999]">
        @if($showSuccess)
            <div wire:key="success-{{ now()->timestamp }}" class="alert alert-success flex flex-row items-center" x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)">
                ...
                <span>Departemen berhasil dihapus!</span>
            </div>
        @endif
...
<script>
    document.addEventListener('livewire:init', () => {
        Livewire.on('confirm-delete', (event) => {
            document.getElementById('modal_delete_departemen').showModal();
        });
        
        Livewire.on('close-delete-modal', () => {
            document.getElementById('modal_delete_departemen').close();
        });
    });
</script>
