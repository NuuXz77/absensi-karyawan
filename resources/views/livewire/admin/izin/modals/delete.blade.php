<div>
    <dialog id="modal_delete_izin" class="modal" wire:ignore.self>
        <div class="modal-box border border-base-300">
            ...
            @if($this->izin)
                ...
                <div class="mt-4 space-y-3">
                    <p class="text-base-content/80">Apakah Anda yakin ingin menghapus pengajuan izin ini?</p>
                    ...
                    <fieldset>
                        ...
                        <label class="input w-full input-bordered flex items-center gap-2">
                            ...
                            <input type="text" value="{{ $this->izin->karyawan->nama_lengkap ?? '-' }}" disabled class="bg-base-200" />
                        </label>
                    </fieldset>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <fieldset>
                            ...
                            <label class="input w-full input-bordered flex items-center gap-2">
                                ...
                                <input type="text" value="{{ \Carbon\Carbon::parse($this->izin->tanggal_mulai)->format('d/m/Y') }}" disabled class="bg-base-200" />
                            </label>
                        </fieldset>
                        ...
                        <fieldset>
                            ...
                            <label class="input w-full input-bordered flex items-center gap-2">
                                <x-heroicon-o-calendar class="w-4 h-4 opacity-70" />
                                <input type="text" value="{{ \Carbon\Carbon::parse($this->izin->tanggal_selesai)->format('d/m/Y') }}" disabled class="bg-base-200" />
                            </label>
                        </fieldset>
                    </div>
                    ...
                    <fieldset>
                        ...
                        <label class="input w-full input-bordered flex items-center gap-2">
                            ...
                            <input type="text" value="{{ ucfirst($this->izin->status) }}" disabled class="bg-base-200" />
                        </label>
                    </fieldset>
                </div>
    ...
    <div class="toast toast-start z-[9999]">
        @if($showSuccess)
            <div wire:key="success-{{ now()->timestamp }}" class="alert alert-success flex flex-row items-center" x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)">
                ...
                <span>Data izin berhasil dihapus dan saldo telah dikembalikan!</span>
            </div>
        @endif
...
<script>
    document.addEventListener('livewire:init', () => {
        Livewire.on('open-delete-modal', (event) => {
            document.getElementById('modal_delete_izin').showModal();
        });
        
        Livewire.on('close-delete-modal', () => {
            document.getElementById('modal_delete_izin').close();
        });
    });
</script>
