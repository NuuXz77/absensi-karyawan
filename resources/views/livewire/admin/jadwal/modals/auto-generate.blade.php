<div>
            ...
            <form wire:submit.prevent="generate" class="mt-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <fieldset>
                        ...
                        <input type="date" wire:model="tanggal_mulai" class="input input-bordered w-full @error('tanggal_mulai') input-error @enderror" />
                        @error('tanggal_mulai')
                    ...
                    <fieldset>
                        ...
                        <input type="date" wire:model="tanggal_selesai" class="input input-bordered w-full @error('tanggal_selesai') input-error @enderror" />
                    ...
                    <fieldset class="col-span-2">
                        ...
                        <select wire:model.live="departemen_id" class="select select-bordered w-full @error('departemen_id') select-error @enderror">
                    ...
                    <fieldset class="col-span-2">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <label class="cursor-pointer label justify-start gap-3 bg-base-200 rounded-lg p-3 border-2 {{ $shift_pattern === 'single' ? 'border-primary' : 'border-transparent' }}">
                                <input type="radio" wire:model.live="shift_pattern" value="single" class="radio radio-primary" />
                                <div>
                                    <span class="label-text font-semibold">Single Shift</span>
                                    <p class="text-xs opacity-70">Semua karyawan shift yang sama</p>
                                </div>
                            </label>
                            <label class="cursor-pointer label justify-start gap-3 bg-base-200 rounded-lg p-3 border-2 {{ $shift_pattern === 'rotating' ? 'border-primary' : 'border-transparent' }}">
                                <input type="radio" wire:model.live="shift_pattern" value="rotating" class="radio radio-primary" />
                                <div>
                    ...
                    <fieldset class="col-span-2">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($shifts as $shift)
                                <label class="cursor-pointer label justify-start gap-3 bg-base-200 rounded-lg p-3 hover:bg-base-300">
                                    <input type="checkbox" wire:model="selected_shifts" value="{{ $shift->id }}" class="checkbox checkbox-primary" />
                                    <div class="flex-1">
                                        <span class="label-text font-semibold">{{ $shift->nama_shift }}</span>
                                        <p class="text-xs opacity-70">{{ \Carbon\Carbon::parse($shift->jam_masuk)->format('H:i') }} - {{ \Carbon\Carbon::parse($shift->jam_pulang)->format('H:i') }}</p>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                        ...
                        <fieldset class="col-span-2">
                            <div class="alert alert-info">
                                ...
                                <div class="text-sm">
                                    ...
                                    <p>Shift: {{ count($selected_shifts) }} shift {{ $shift_pattern === 'rotating' ? '(rotating)' : '(single)' }}</p>
                                </div>
                            </div>
                        </fieldset>
                    @endif
                    ...
                    <div class="col-span-2 flex justify-end gap-3 mt-4">
                        ...
                        <button type="submit" class="btn btn-success btn-sm gap-2" wire:loading.attr="disabled" wire:target="generate">
    ...
    <div class="toast toast-start z-[9999]">
        @if($showSuccess)
            <div wire:key="success-{{ now()->timestamp }}" class="alert alert-success flex flex-row items-center" x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)">
...
<script>
    document.addEventListener('livewire:init', () => {
        Livewire.on('open-auto-generate-modal', () => {
            document.getElementById('modal_auto_generate_jadwal').showModal();
        });
        
        Livewire.on('close-auto-generate-modal', () => {
            document.getElementById('modal_auto_generate_jadwal').close();
        });
    });
</script>
