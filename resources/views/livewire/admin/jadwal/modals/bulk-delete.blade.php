<div x-data="modal('modal_bulk_delete_jadwal')">
    <button class="btn btn-error btn-sm gap-2" @click="openModal">
                ...
                <div class="grid grid-cols-2 gap-4">
                        ...
                        <fieldset class="col-span-2">
                            <div class="alert alert-warning">
                                ...
                                <div class="text-sm">
                                    ...
                                    @if ($departemen_id)
                                        <p>Departemen:
                                            {{ $departemens->firstWhere('id', $departemen_id)->nama_departemen ?? '-' }}
                                        </p>
                                    @else
                                        <p>Departemen: Semua</p>
                                    @endif
                                    <p class="text-error font-bold mt-2">Total {{ $previewCount }} jadwal akan dihapus!
                                    </p>
                                </div>
                            </div>
                        </fieldset>
                    ...
                    <fieldset class="col-span-2">
                        ...
                        <input type="text" wire:model.live="confirmText"
                            ...
                            placeholder="Ketik HAPUS untuk konfirmasi" autocomplete="off" />
                        @error('confirmText')
                            <p class="text-error text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </fieldset>
                    ...
                    <div class="col-span-2 flex justify-end gap-3 mt-4">
                        ...
                        <button type="submit" class="btn btn-error btn-sm gap-2" wire:loading.attr="disabled"
                            wire:target="delete" @disabled($previewCount === 0 || strtoupper($confirmText) !== 'HAPUS')>
